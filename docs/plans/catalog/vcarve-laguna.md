...
- **Pocketing is raster-only with no islands or offset/HSM clearing (P0/P1).** `generatePocket2dLines` is a single-ring zig-zag; no interior islands, no spiral/offset clearance. ~~the adaptive/trochoidal kinds lack a 2D engine~~ — RESOLVED by Wave 3i (islands + offset spiral) and Stack B v1 (adaptive-lite 2D engine, `cnc_adaptive` now in the Laguna op set).
