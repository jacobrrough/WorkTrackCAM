...
| High     |    4 | **dev-only, DEFERRED** — esbuild dev-server advisory GHSA-gv7w-rqvm-qjhr via the `vite` / `electron-vite` / `@vitejs/plugin-react` chain (4 nodes). A 0.28.1 `overrides` bump was tried + **reverted** 2026-06-16: esbuild 0.28 breaks `npm run dev` (destructuring-transform incompat with vite 6.4.3 optimizeDeps). Never ships. See Deferred below. |
...
| **Total**| **4** | `npm audit --omit=dev` reports **0** (runtime release gate GREEN); full `npm audit` reports **4 high**, all the dev-only esbuild chain (deferred). |

**Runtime / user-shipping advisory count: 0 (release gate GREEN). Full dev+prod advisory count: 4** — the DOMPurify runtime low (GHSA-gvmj-g25r-r7wr) was **closed 2026-06-16** via an `overrides` bump; the esbuild dev-chain high (GHSA-gv7w-rqvm-qjhr) is **deferred** (a 0.28 override was tried + reverted — it breaks `npm run dev`; see Deferred).
...
re-verified clean). The esbuild dev-server high that remained after that fix (GHSA-gv7w-rqvm-qjhr) is **deferred**
(dev-only) — a 0.28.1 `overrides` bump was tried 2026-06-16 and reverted because it breaks `npm run dev`; see Deferred.
...
### High — esbuild dev-server GHSA-gv7w-rqvm-qjhr — DEV-ONLY, DEFERRED
- **Chain:** `esbuild` → `vite` 6.4.3 / `electron-vite` 3.1.0 / `@vitejs/plugin-react` (4 high nodes in full `npm audit`). Fixed upstream in esbuild 0.28.1.
- **Why it does NOT ship:** the advisory is in esbuild's **dev server** (a page can send it cross-origin requests and read the response). It affects `npm run dev` only — `electron-vite build` emits static bundles with no dev server. `npm audit --omit=dev` = **0** confirms nothing shipped is affected.
- **Why deferred — TRIED + REVERTED 2026-06-16:** forcing esbuild `^0.28.1` via `overrides` cleared the advisory AND passed `electron-vite build` + the full vitest suite — **but broke `npm run dev`**: esbuild 0.28 errors `"Transforming destructuring to the configured target … is not supported yet"` across ~1162 deps in vite 6.4.3's optimizeDeps (0.28 is outside vite's declared `^0.25.0`). The build + test gates missed it because neither exercises the dev dep-optimizer — only a hands-on `npm run dev` surfaced it (the verify-it-for-real lesson). Reverted to esbuild 0.25.12 (vite's range); dev restored.
- **Re-evaluation trigger:** a `vite`/`electron-vite` major that itself depends on esbuild ≥0.28, OR an esbuild 0.28.x that fixes the optimizeDeps destructuring transform — and add an `npm run dev` smoke to the release gate so a repeat is caught.
- **Status:** **DEFERRED** (dev-only). Runtime release gate (`npm audit --omit=dev` = 0) unaffected.
...
- `esbuild` is NOT overridden — it tracks `vite`'s declared `^0.25.x` (currently 0.25.12). A 0.28.1 override was tried 2026-06-16 to clear GHSA-gv7w-rqvm-qjhr and reverted (broke `npm run dev`); see Deferred advisories.
