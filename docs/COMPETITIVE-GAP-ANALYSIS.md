...
| **9** | **True-shape nesting for Laguna sheet jobs (VCarve Pro parity)** | Laguna Swift's whole purpose is full-sheet plywood/MDF. Without nesting, the owner re-arranges by hand and wastes material. Cuts cost per sheet meaningfully. | L | DIRECT Laguna | New `src/main/nesting/true-shape.ts` (port a permissive-licensed library like Deepnest's NFP algorithm) plus a "Nest" button in the contour/profile operation panel |
...
2. **True-shape nesting for Laguna sheet jobs (Gap #9)**. Port Deepnest's NFP-with-genetic-algorithm (MIT-licensed) into a TypeScript or Python sidecar service. Wire into the contour-operation panel as a "Nest selected parts on stock" action. **Impact**: turns the Laguna workflow from "draw, lay out, post" into "draw, nest, post" — the workflow every sign-shop/cabinet-shop expects. **Effort**: 2–3 weeks if porting; <1 week if shelling out to `deepnest-cli`.
