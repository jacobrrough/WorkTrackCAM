 ...
 *      carries a real local AABB (its `geometryDimensions`, or a live-geometry
 *      override), the engine's OBB narrow phase activates and rotation-inflated
 *      bbox false positives are cleared (`fidelity: 'bbox+narrow'`); otherwise the
 *      result is the pure conservative broad phase (`fidelity: 'bbox'`).
 ...
 *       · a part with a real local AABB (its `geometryDimensions`, or a live-
 *         geometry override via {@link localAabbFromGeometry}) is measured at its
 *         TRUE extent and refined by the engine's OBB narrow phase — rotation-
 *         inflated false positives are cleared (`fidelity: 'bbox+narrow'`);
 *       · a part with NO dims falls back to a fixed {@link NOMINAL_HALF_EXTENT_MM}
 *         cube centred at its placement — the honest "are these parts at ~the same
 *         place" heuristic, and the narrow phase keeps any such pair conservatively.
...
import {
  ...
  type AssemblyPartGeometryDimensions,
} from '../../shared/assembly-part'
 ...
 * no sidecar round-trip and no protocol change. Returns the box in the
 * {@link AssemblyPartGeometryDimensions} `aabbMin` / `aabbMax` shape the shared
 * schema + narrow phase consume.
 *
...
export function localAabbFromGeometry(
...
): AssemblyPartGeometryDimensions | null {
  ...
  return { aabbMin: [min.x, min.y, min.z], aabbMax: [max.x, max.y, max.z] }
}

/**
 * A part's real local AABB, sourced from its persisted {@link AssemblyPart.geometryDimensions}
 * (the tessellation bbox captured when the part was added) via the shared
 * {@link assemblyPartLocalAabb}. `undefined` when the row carries no (or a
 * malformed) box — the interference check then uses the nominal cube for that
 * part and the narrow phase treats it as "no dims" (kept conservatively).
 */
function realLocalBox(part: AssemblyPart): InterferencePart['localBox'] | undefined {
  return assemblyPartLocalAabb(part)
}
 ...
 *      live geometry), else
 *   2. the part's persisted `geometryDimensions` bbox, else
 ...
 * `geometryDimensions`. Parts with only the nominal cube are OMITTED so the
 * narrow phase treats them as "no dims" (keeps the pair conservatively rather than
 * refining a fake extent). The transform is carried through so the OBB SAT rotates
 * each real box exactly as the broad phase does. Returns an empty map when no part
 * carries real geometry (the check then stays byte-identical to the pure bbox path).
 */
 ...
 *
 * When any part carries a real local AABB (its persisted `geometryDimensions`, or a
 * live-geometry override in `boxes`), the engine's OBB **narrow phase** activates
 * via {@link detectInterferencesWithDims}: rotation-inflated bbox false positives
 * are cleared and `fidelity` becomes `'bbox+narrow'`. When NO part carries real
 * geometry the map is empty, so the result is byte-identical to the conservative
 * bbox-only path (`fidelity: 'bbox'`) — no regression for un-hydrated assemblies.
 * A part missing dims degrades to its nominal cube for the broad phase and is kept
 * conservatively by the narrow phase (never crashes, never a false negative).
 *
 ...
 * persisted `geometryDimensions` for both the broad and narrow phases.
 */
