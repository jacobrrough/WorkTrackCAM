 ...
 * with a part hydrated from its durable geometry source's cached local bounds
 * (see the renderer seam's `geometrySourceRef.cachedBounds` adapter).
 */
 ...
 * assembly whose rows carry a cached local box (the renderer seam derives it from
 * `geometrySourceRef.cachedBounds`) gets the refinement for free.
 */
