...
- **P5 CUTOVER (done):** the legacy ShopApp shell was retired. Deleted: `ShopApp.tsx`, `AppHeader.tsx`/`ShopBrandBar`, `NavRail.tsx`, `AppStatusBar.tsx`, `EnvironmentSplash.tsx`, `control-center.css`, and the `__APP_SHELL__` dark-launch flag. `main.tsx` now renders `WorkTrack3DApp` (the new CAD-first shell in `src/renderer/app/`) as the sole shell — there is no `WT_SHELL` flag anymore. The retired `shop-app-toolbar-button-types.test.ts` button-type pin was replaced by `src/renderer/app/__tests__/new-shell-button-types.test.ts` (walks `src/renderer/app/**`).
- `src/renderer/app/` — **the WorkTrack3D shell**: `WorkTrack3DApp` → `AppShell` (grid frame) → `TopBar` (brand · New/Open/Save · machine status + **E-stop**) · `WorkspaceNav` (Design · Assemble · Manufacture · Drawings · Workshop · Utilities) · `WorkspaceHost` (renders one reused workspace; default route `design`) · `StatusBar`. E-stop button calls `window.fab.machine.estop({machineId})` — the IPC dispatch in `src/main/ipc-machine.ts` is unchanged (K2 → Moonraker `/printer/emergency_stop`; Carvera/Laguna → physical-e-stop advisory toasts).
...
- `src/renderer/design/DesignWorkspace.tsx` — **CAD Design workspace** — a top-level workspace in the new shell and the CAD-first **default route** (`WorkspaceHost`). Composes `CadQueryEditor` (left), 3D viewport (center), `FeatureTree` (right). "Send to CAM" exports STL, switches to the active machine env, and auto-imports the STL into the first plate. In the new shell it is wrapped by `DesignSessionProvider` (mounted in `WorkspaceHost` via `DesignWorkspaceHost`) which feeds the editable feature timeline + kernel-op editors. (The old fullscreen-overlay / `designOpen`-in-ShopApp model was retired with the CAD-first pivot + P5 cutover.)
...
- `src/shared/app-keyboard-shortcuts.ts` — single source of truth for keyboard shortcuts (1–6 for the workspace nav `WorkspaceNav`, F1 for Help, Ctrl+K palette, Ctrl+Shift+D for the Design workspace, Ctrl+Shift+? for the shortcuts dialog). When wiring a new shortcut, add it here so the shortcuts dialog documents it.
...
- `docs/SECURITY.md` — security posture, closed/deferred advisory tracking, and the release-gate cadence. **Every cycle that touches `package.json` / `package-lock.json` MUST run `npm audit`** and either close or document any new advisory here. `npm audit --omit=dev` MUST return 0 advisories before any release build — that is the runtime security gate. Live source of truth for *open* advisories is the GitHub Dependabot dashboard linked in the doc; this file tracks dispositions and the reason each deferred advisory is deferred.
